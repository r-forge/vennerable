...
makeme <- function() {
	...
	Sweave(driver="weaver","AWFE.Rnw",stylepath=FALSE,use.cache=TRUE)
}
makeme()
@
...
Edwards CITE starts by constructing Venn sets on a sphere and then projecting onto the plane.
...
<<awefebuilderdef,cache=TRUE,eval=TRUE>>=

<<awefebuilder,cache=TRUE,eval=FALSE>>=
saveTD("battle",Setlist =SetBoundaries[["battle"]],dataFile=dataFile)
saveTD(type="AWFEscale",Setlist =SetBoundaries[["AWFEscale"]],dataFile=dataFile)
saveTD("AWFE",Setlist =SetBoundaries[["AWFE"]][1:8],dataFile=dataFile)
@
