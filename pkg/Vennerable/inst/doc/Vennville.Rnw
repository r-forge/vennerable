%**************************************************************************
%
% # $Id: Vennville.Rnw,v 1.7 2007/10/16 10:59:52 js229 Exp $
...
\section{The edgeSector object)

<<defedgeSector>>=
nodeList <- list(p1=matrix(1:2,ncol=2),p2=matrix(2:3,ncol=2))
lh <- newEdgeSector(Set=1,centre=c(-1,5),hand=1,from="p1",to="p2")
DV <- new("VennDrawing",nodeList=nodeList)
grid.newpage();pushViewport(plotViewport(c(1,1,1,1)))
makevp.eqsc(c(-7,7),c(-5,10))
grid.xaxis()
grid.yaxis()
PlotNodes(DV)
xy <- Arc.to.xy(DV,lh); grid.lines(xy[,1],xy[,2],default.units="native",arrow=arrow())
lh@hand <- -1
xy <- Arc.to.xy(DV,lh); grid.lines(xy[,1],xy[,2],default.units="native",arrow=arrow(),gp=gpar(col="red"))
@


\section{The DrawnVenn object)
Because the graph package supports only singly-connected edges, 
and because orientation of an arc can be different when drawing it
as part of a face and part of a Set, the edge data is stored only in the face data.
<<>>=

nodeList <- list(p1=matrix(1:2,ncol=2),p2=matrix(2:3,ncol=2),p3=matrix(c(-1,0),ncol=2))

sectorfromto <- function(sector,from,to) {
	sector@from <- from
	sector@to <- to
	sector
}

lh <- new("edgeSector",Set=1,centre=c(-1,5),hand=1);lh <- sectorfromto(lh,"p1","p2")
rh <- new("edgeSector",Set=2,centre=c(4,0),hand=1)
otherhand <- function(eSector){eSector@hand <- -eSector@hand;eSector}

el <- new("edgeLines",Set=3,from="p1",to="p3",xy=matrix(c(1,2,-0.5,0,-1,0),ncol=2,byrow=T))
edgeList <- list("p1|p2|1"=sectorfromto(lh,"p1","p2"),
                 "p2|p1|1"=sectorfromto(lh,"p2","p1"),
                 "p1|p2|2"=sectorfromto(rh,"p1","p2"),
                 "p2|p1|2"=sectorfromto(rh,"p2","p1"),
		     "p1|p3|3"=el,
		     "p3|p1|3"=new("edgeLines",Set=3,from="p3",to="p1",xy=matrix(c(-1,0,1,2),ncol=2,byrow=T))
			)

faceList <- list("100"=c("p1|p2|1","-p2|p1|2"),"110"=c("p2|p1|2","p1|p2|2"),"010"=c("p2|p1|2","-p1|p2|1"),"001"=c("p1|p3|3","p3|p1|3"))

setList <- list("1"=c("p1|p2|1","p2|p1|1"),
	"2"=c("p1|p2|2","p2|p1|2"),
	"3"=c("p1|p3|3","p3|p1|3")
	)

DV <- new("VennDrawing",nodeList =nodeList ,edgeList =edgeList ,setList=setList,faceList=faceList)
...
PlotFaces(DV)
PlotSetBoundaries(DV,gp=gpar(lwd=2,col=c("red","blue","green")))
PlotNodes(DV)
...
\begin{figure}[H]
  \begin{center}
<<ccomboutranspfaces,fig=TRUE>>= 
grid.newpage()
r <- C3combo@r
xy <- cbind(C3combo@x,C3combo@y)
#intersections <- do.call(rbind,
#		list(.intersectionpoints(r,xy,1,2),
#			.intersectionpoints(r,xy,2,3),
#			.intersectionpoints(r,xy,3,1)))
#grid.points(intersections[,1],intersections[,2],default.units="native")
C3DV <- ThreeCircles2(r,xy,as(C3combo,"Venn"))
	C3DV <- .square.universe(C3DV,doWeights=TRUE)
PlotVennGeometry(C3DV,show=list(Faces=TRUE,intersection.weight=FALSE))
downViewport("Vennvp")
PlotNodes(as(C3DV,"DrawnVenn"))
diagram <- as(C3DV,"DrawnVenn")@diagram
for (from in nodes(diagram)[1]) {
	for (to in edges(diagram,from)[[1]][1]) {
		arc <- 	 edgeData(diagram,from,to,"arc")[[1]]
		arc.Set <- arc@Set
		arc.xy <- Arc.to.xy(diagram,from,to,arc,nintervals=100)
		grid.lines(arc.xy[,1],arc.xy[,2],default.units="native",arrow=arrow())
	}
}

@

  \caption{ 3D Venn diagram. Faces not yet implemented }
  \end{center}
\end{figure}
...
TD <- compute.AWFE(Venn(NumberOfSets=4))
library(graph); library(RBGL); source("../../R/ChowRuskey.R")
TDgr <- TDtograph(TD)$graph
sTDgr <- scythegr(TDgr)
ray.to.point.data <- my.tsort(sTDgr)
CR4 <- compute.CR(V=V4,doWeights=TRUE)
CRW4 <- compute.CR(V=V4,doWeights=FALSE)
CRW3 <- compute.CR(V=V3,doWeights=FALSE)
@

\begin{figure}[H]\begin{center}
<<shoVDAWFE4x,fig=TRUE,cache=TRUE,eval=FALSE>>=
grid.newpage();pushViewport(plotViewport(c(1,1,1,1)))
makevp.eqsc(4*c(-1,1),4*c(-1,1))
grid.xaxis();grid.yaxis()
PlotSetBoundaries(CR4 )
PlotFaces(CR4 )
.PlotFaceNames.TissueDrawing (CR4 )
@
\caption{4th order AWFE diagram}
 \end{center}\end{figure}
...
Author & Jonathan Swinton
\\
CVS id of this document & ${}$Id: Vennville.Rnw,v 1.7 2007/10/16 10:59:52 js229 Exp ${}$.
\\
Generated on & \today
\\
R version & 
