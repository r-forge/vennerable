...
\usepackage{Sweave}
\SweaveOpts{prefix.string=Venndrawnfig,cache=TRUE,debug=TRUE,eps=FALSE,echo=TRUE,pdf.version=1.4}
\usepackage{natbib}
\usepackage{mathptmx}
\usepackage{rotating} 
\usepackage{float} 
...
grid.xaxis()
grid.yaxis()
shoar <- function(TN2) {for (ix in 1:length(TN2@edgeList)) {xy <- .edge.to.xy(TN2@edgeList[[ix]]); grid.lines(xy[,1],xy[,2],default.units="native",arrow=arrow())}}
#shoar(TN2)
...
PlotNodes(VDI)
lapply(VDI@edgeList,function(lh){xy <- .edge.to.xy(lh); grid.lines(xy[,1],xy[,2],default.units="native",arrow=arrow())})
...
PlotNodes(VDO)
lapply(VDO@edgeList,function(lh){xy <- .edge.to.xy(lh); grid.lines(xy[,1],xy[,2],default.units="native",arrow=arrow())})
...
grid.yaxis()
PlotFaces(TM3)
PlotSetBoundaries(VDC1);PlotSetBoundaries(VDC2);
PlotNodes(VDC1);PlotNodes(VDC2);
.PlotFaceNames.TissueDrawing (TM3)
PlotNodes(TM3)
...
x <- c(0.000000,  1.384666, -1.028597 )
 y <- c(1.49270533, -0.55257134, -0.02662434 )
	centres <- matrix(c(x,y),ncol=2,byrow=FALSE)
	VDC1 <- newTissueFromCircle(centres[1,],radius=r[1],Set=1,nodes=6); 
	VDC2 <- newTissueFromCircle(centres[2,],radius=r[2],Set=2,nodes=2); 
	TM <- addSetToDrawing (drawing1=VDC1,drawing2=VDC2,set2Name="Set2")
	VDC3 <- newTissueFromCircle(centres[3,],radius=r[3],Set=3); 
